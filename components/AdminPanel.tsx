          ...
          <div className="flex-grow overflow-y-auto p-12 custom-scrollbar">
            ...
            {activeTab === 'catalog' && editingGame && (
              <div className="max-w-4xl mx-auto space-y-10 pb-12 animate-fade">
                ...
                {/* Main Assets & Links */}
                <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 space-y-8">
                   <h4 className="text-[10px] font-black text-[#0072ce] uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><LinkIcon className="w-3.5 h-3.5" /> Core Assets (Primary)</h4>
                   <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2 flex items-center gap-2"><Download className="w-3 h-3 text-emerald-500" /> Primary Download URL (The main file)</label>
                        <input type="text" value={editingGame.downloadUrl || ''} onChange={e => setEditingGame({...editingGame, downloadUrl: e.target.value})} className="w-full p-5 bg-white border border-slate-200 rounded-2xl font-bold outline-none focus:border-emerald-500 transition-all" placeholder="Direct link to main .pkg or .zip" />
                      </div>
                      <div className="space-y-2">
                        ...
                        <input type="text" value={editingGame.imageUrl || ''} onChange={e => setEditingGame({...editingGame, imageUrl: e.target.value})} className="w-full p-5 bg-white border border-slate-200 rounded-2xl font-bold outline-none focus:border-blue-500 transition-all" placeholder="Link to game poster image" />
                      </div>
                   </div>
                </div>
                ...
                <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 space-y-8">
                  <div className="space-y-6">
                    <div className="flex justify-between items-center border-b pb-4">
                      <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2 flex items-center gap-2"><Layers className="w-3 h-3 text-[#0072ce]" /> Versioned Nodes (Updates)</label>
                      <button onClick={() => setEditingGame({...editingGame, updates: [...(editingGame.updates || []), {version:'', firmware:'', downloadUrl:''}]})} className="text-[9px] font-black text-[#0072ce] flex items-center gap-2 hover:underline"><Plus className="w-3 h-3"/> New Node</button>
                    </div>
                    <div className="space-y-3">
                      {(editingGame.updates || []).map((upd, idx) => (
                        <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 grid grid-cols-12 gap-4">
                          ...
                          <input placeholder="Update Link" className="col-span-7 p-3 bg-slate-50 rounded-lg text-xs font-bold" value={upd.downloadUrl} onChange={e => {
                ...
                <div className="flex gap-4">
                  <button onClick={handleSaveGame} disabled={isSaving} className="flex-grow py-6 bg-[#0072ce] text-white rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-2xl shadow-blue-500/20 active:scale-95">
                    {isSaving ? 'Synchronizing...' : (editingGame.id ? 'Save Changes' : 'Commit New Entry')}
                  </button>
                  <button onClick={() => setEditingGame(null)} className="px-12 py-6 bg-slate-100 text-slate-400 rounded-[2rem] font-black uppercase text-[11px] hover:bg-slate-200">Cancel</button>
                </div>
              </div>
            )}
                ...
                {games.map(g => (
                  <div key={g.id} className="p-6 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex justify-between items-center group hover:bg-white transition-all">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setEditingGame(g)} className="text-[10px] font-black uppercase tracking-widest text-[#0072ce] px-8 py-3 bg-white border border-slate-100 rounded-xl hover:bg-[#0072ce] hover:text-white transition-all shadow-sm">Edit Entry</button>
                      <button 
                        onClick={async () => {
                          if(confirm(`Are you sure you want to delete ${g.title}?`)) {
                            await supabase.from('games').delete().eq('id', g.id);
                            onUpdateGame();
                            showToast('info', 'Asset Purged', 'Game removed from vault.');
                          }
                        }}
                    ...
                    <div key={rep.id} className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex justify-between items-center hover:bg-white transition-all">
                       <div className="flex items-center gap-6">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase">Reported on {new Date(rep.created_at).toLocaleDateString()}</p>
                        </div>
                       </div>
